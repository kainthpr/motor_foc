...
volatile float m =0.3;  // commanded vector magnitude
float dir = 1;
int overflow = 0;
float ki = 0.5;
float kp = 40;
...
//			int length = sprintf(tx_buffer, "%d,%d,%f,%d\r\n", vel, error, m, angle);
			int length = sprintf(tx_buffer, "%d,%f,%d,%f,%u,%u,%u\r\n", vel, integral, angle, m, adc[0], adc[1], adc[2]);
